...
.data
prompt_user:    .asciiz "\nVuoi pescare o fermarti? (1 = draw, 2 = stand): "
user_win:       .asciiz "\nHai vinto!\n"
comp_win:       .asciiz "\nHai perso!\n"
draw:           .asciiz "\nPareggio!\n"
your_hand:      .asciiz "\nLa tua mano: "
...
computer_turn:
	...
	li $t9, 21
	bne $t0, $t9, hand_check #se player hand = 21 il computer deve fare 21 per provare a pareggiare 
	li $t7 , 21
	j end_strat
	
	hand_check:
	li $t7, 17
	bgt $t0, $t9, end_strat # se player hand >21 al computer basta fare 17
	blt $t0, $t7, end_strat # se player hand < 17 il computer deve fare almeno 17
	move $t7, $t0 # se player hand é compresa tra 17 e 20 il computer deve battere o pareggiare quel valore
	end_strat:
...
draw_card:
	...
	li $a1, 52         # Genera un numero casuale tra 0 e 52 in a0
	syscall
	la $t4, deck
...
#incrementa il conteggio degli assi del giocatore
inc_player_ace:
	addi $t2, $t2, 1
	jr $ra

#incrementa il conteggio degli assi del computer
inc_computer_ace:
	addi $t3, $t3, 1
	jr $ra
...
adjust_player_aces:
	...
	sub $t0, $t0, $t6
	subi $t2, $t2, 1
	bgtz $t2, adjust_player_aces # Continua ad aggiustare se ci sono più assi (loop)
